...
- App name: `minh-personal-website-frontend`

Initial one-time setup (if the `heroku` remote is missing):

```bash
# From project root
heroku git:remote -a minh-personal-website-frontend
```

Environment variables (set/update as needed):

```bash
# Example
heroku config:set REACT_APP_API_BASE_URL=https://your-backend-domain
```

Deploy current `master` branch to Heroku:

```bash
git pull origin master
git add -A
git commit -m "Deploy" # if there are changes
...
heroku open                       # open the deployed app in browser
heroku releases                   # list releases
heroku rollback vNN               # rollback to a previous release
```

Notes:

- Build is triggered by Heroku. The `Procfile` runs `serve -s build` to host the CRA `build/` output.
- The project uses Node 16.x on Heroku as defined in `package.json`.
